...
def get_patients_from_gcp(condition_code, gender, age_range):
    ...
    query = f"gender={gender}&birthdate=ge{min_birth_date}&birthdate=le{max_birth_date}&condition.code:text={condition_code}"
...
def get_conditions_from_gcp(condition_search, gender, age_range):
    ...
    min_birth_date, max_birth_date = calculate_birth_date_range(age_range)
    query = (
        f"subject:Patient.birthdate=ge{min_birth_date}"
        f"&subject:Patient.birthdate=le{max_birth_date}&code:text={condition_search}"
    )
